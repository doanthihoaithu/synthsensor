...
def generate_background_function(settings_cfg, generation_cfg, mode='train'):
...
def generate_train_and_test_data_at_once(settings_cfg, generation_cfg, mean_list, sd_list):
    ...
    if generation_cfg.add_background:
        ...
        bg = generate_background_function(settings_cfg, generation_cfg, mode='both')
    ...
    noise = multivariate_normal(mean=[0]*num_sensors, cov=cov).rvs(size=num_data_point_per_batch*num_testing_batches+ num_training_data_points)
    log.info(f'Noise are generated with shape {noise.shape}')
...
def generate_data_function_from_cfg(settings_cfg, generation_cfg):
    ...
    mean_range = generation_cfg.mean_range
    # Noise strength in sensor
    sd_range = generation_cfg.sd_range


    mean_list = [random.sample(mean_range, 1)[0] for _ in range(num_sensors)]
    sd_list = [random.sample(sd_range, 1)[0] for _ in range(num_sensors)]
    ...
    #     dfs.append(df)
    testing_df, training_df = generate_train_and_test_data_at_once(settings_cfg, generation_cfg, mean_list, sd_list)
